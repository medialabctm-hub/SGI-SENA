...
import { FiCalendar, FiClock, FiInbox, FiMapPin, FiMonitor, FiRefreshCw, FiUser } from 'react-icons/fi'
...
function formatElapsed(minutes) {
  ...
  if (!Number.isFinite(totalMinutes) || totalMinutes < 1) return 'Hace unos minutos'
...
function ScheduleDetails({ session }) {
  ...
  const startTime = formatTime(session.hora_inicio || session.clase_hora_inicio)
...
export default function EquiposPrestados() {
  ...
  async function fetchSessions(isRefresh = false) {
    ...
    try {
      ...
      const data = await parseApiResponse(res, 'No se pudieron cargar los equipos prestados')
      setSessions(Array.isArray(data.sesiones) ? data.sesiones : [])
    } catch (err) {
      setToast({ message: buildErrorMessage(err, 'No se pudieron cargar los equipos prestados'), type: 'error' })
      setSessions([])
    } finally {
      setLoading(false)
      setRefreshing(false)
  ...
  useEffect(() => {
    ...
    const unsubscribeAutoservicio = subscribe('equipo:autoservicio_iniciado', refreshSessions)
    const unsubscribeAsignacionCreated = subscribe('asignacion:created', refreshSessions)
    const unsubscribeAsignacionDeleted = subscribe('asignacion:deleted', refreshSessions)
        ...
        <main className="dashboard-main">
          ...
          <div className="form-equipos form-modern prestados-page">
            <div className="form-header">
              ...
              <div className="form-header-content">
                ...
                <p className="form-header-subtitle">
                  Consulta los equipos que están en uso, el ambiente y la programación asociada.
                </p>
              </div>
              <button
                type="button"
              ...
              >
                <FiRefreshCw size={16} className={refreshing ? 'prestados-spin' : ''} />
                Actualizar
              </button>
            </div>
            ...
            <div className="prestados-summary">
              ...
              {sessions.length === 1 ? ' equipo está prestado actualmente' : ' equipos están prestados actualmente'}
            </div>
                  ...
                  <tbody>
                    {sessions.map(session => (
                      <tr key={session.id_historial}>
                        <td>
                          <strong>{session.codigo_inventario || `Equipo ${session.codigo_equipo}`}</strong>
                          <div className="prestados-muted">
                            {[session.equipo_tipo, session.equipo_modelo].filter(Boolean).join(' · ') || '-'}
                          </div>
                        </td>
                        <td>
                          ...
                          {session.codigo_ambiente && (
                            <div className="prestados-muted">{session.codigo_ambiente}</div>
                          )}
                        </td>
                        <td>
                          <div className="prestados-icon-text">
                            <FiUser size={15} />
                            {session.usuario_nombre || 'Usuario no identificado'}
                          </div>
                          {session.usuario_cedula && (
                            <div className="prestados-muted">Documento: {session.usuario_cedula}</div>
                          )}
                        </td>
                        <td><ScheduleDetails session={session} /></td>
                        <td>
                          <div>{formatDateTime(session.fecha_hora_inicio)}</div>
                          <div className="prestados-elapsed">Hace {formatElapsed(session.minutos_transcurridos)}</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
